...
## Step-by-Step (assuming an initial flow)

For one window *i*, given a starting flow *u*:

1. **Gather the window.** Collect the n<sub>i</sub> events in `[T`<sub>i</sub>`, T`<sub>i+1</sub>`]` around the feature.

2. **Build the reference.** Forward-propagate the previous window's events to T<sub>i</sub> to get the landmark approximation:
   > **l̃<sub>j</sub><sup>i−1</sup> = { x + (T<sub>i</sub> − t) · u<sub>i−1</sub>  |  (x, t) ∈ W<sub>i−1</sub> }**

3. **E-step — weights.** For each event, compute its soft association to every reference as a normalized Gaussian:
   > **r<sub>kj</sub> = φ( x<sub>k</sub> − t̄<sub>k</sub>u ; l̃<sub>j</sub><sup>i−1</sup>, Σ ) / Σ<sub>j′</sub> φ( x<sub>k</sub> − t̄<sub>k</sub>u ; l̃<sub>j′</sub><sup>i−1</sup>, Σ )**

4. **M-step — flow.** With the weights fixed, solve the closed form for the flow:
   > **u = ( Σ<sub>k</sub> Σ<sub>j</sub> r<sub>kj</sub> (x<sub>k</sub> − l̃<sub>j</sub><sup>i−1</sup>) t̄<sub>k</sub> ) / ( Σ<sub>k</sub> Σ<sub>j</sub> r<sub>kj</sub> t̄<sub>k</sub><sup>2</sup> )**

5. **Check the cost.** Evaluate the total weighted residual:
   > **cost = Σ<sub>k</sub> Σ<sub>j</sub> r<sub>kj</sub> ‖ (x<sub>k</sub> − t̄<sub>k</sub>u) − l̃<sub>j</sub><sup>i−1</sup> ‖²**

   If cost > threshold, go back to step 3. Otherwise the flow has converged.

6. **Advance.** Use the converged *u* to move the feature into the next window, then increment *i* and return to step 1.
